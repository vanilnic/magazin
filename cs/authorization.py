...
class main(SQL):
    ...
    def login(self):
        ...
        qwer = self.conn.cursor()
        qwer.execute(('SELECT * FROM user WHERE username = ? and password = ?'), [(self.username.get()), (self.password.get())])
        result = qwer.fetchall()
    ...
    def new_user(self):
        ...
        qwer = self.conn.cursor()
        qwer.execute(('SELECT username FROM user WHERE username = ?'), [(self.n_username.get())])
        result = qwer.fetchall()
        ...
        qwer.execute(insert, [(self.n_username.get()), (self.n_password.get())])
        self.conn.commit()
